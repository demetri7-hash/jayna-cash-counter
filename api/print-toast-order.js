...
async function generateOrderReceiptPDF(order, lineItems) {
  ...
  if (order.delivery_date) {
    ...
    doc.text(`Date: ${deliveryDate.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}`, 50, yPos);
    yPos += 15;
  }
  ...
  if (order.delivery_time) {
    ...
    const dueTimeStr = dueTime.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true });
    doc.setFont('helvetica', 'bold');
    ...
    const leaveTimeStr = leaveTime.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true });
  ...
  doc.setFontSize(8);
  doc.setFont('helvetica', 'italic');
  doc.text(`Printed: ${new Date().toLocaleString('en-US')}`, 40, 770);
