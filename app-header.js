  ...
  const headerHTML = `
    ...
    <div class="header" style="padding-top: 20px;">
      ...
      <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; margin-top: 20px; margin-bottom: 20px;">
        ...
        <button onclick="checkManagerPassword()" style="display: block; width: 100%; padding: 14px; background: var(--gray-900); color: white; border: 2px solid var(--gray-900); text-align: center; font-size: 13px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; cursor: pointer; transition: all 0.15s ease; grid-column: 1 / -1;">MANAGER</button>
      </div>

      <!-- Manager Password Modal -->
      <div id="managerPasswordModal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.8); z-index: 99999; align-items: center; justify-content: center;">
        <div style="background: white; padding: 32px; max-width: 400px; width: 90%; border: 3px solid var(--gray-900);">
          <h2 style="font-size: 18px; font-weight: 700; text-transform: uppercase; margin-bottom: 20px; text-align: center;">MANAGER ACCESS</h2>
          <p style="font-size: 13px; color: var(--gray-600); margin-bottom: 20px; text-align: center;">Enter manager password to continue</p>
          <input type="password" id="managerPasswordInput" placeholder="Enter password" style="width: 100%; padding: 12px; border: 2px solid var(--gray-300); font-size: 14px; margin-bottom: 20px; box-sizing: border-box;" onkeypress="if(event.key === 'Enter') submitManagerPassword()">
          <div style="display: flex; gap: 12px;">
            <button onclick="closeManagerPasswordModal()" style="flex: 1; padding: 12px 24px; background: var(--gray-200); color: var(--gray-900); border: 2px solid var(--gray-300); font-size: 13px; font-weight: 700; text-transform: uppercase; cursor: pointer;">CANCEL</button>
            <button onclick="submitManagerPassword()" style="flex: 1; padding: 12px 24px; background: var(--gray-900); color: white; border: 2px solid var(--gray-900); font-size: 13px; font-weight: 700; text-transform: uppercase; cursor: pointer;">SUBMIT</button>
          </div>
          <div id="managerPasswordError" style="color: #dc2626; font-size: 12px; font-weight: 600; margin-top: 12px; text-align: center; display: none;"></div>
        </div>
      </div>

      <script>
        function checkManagerPassword() {
          document.getElementById('managerPasswordModal').style.display = 'flex';
          document.getElementById('managerPasswordInput').value = '';
          document.getElementById('managerPasswordError').style.display = 'none';
          setTimeout(() => document.getElementById('managerPasswordInput').focus(), 100);
        }

        function closeManagerPasswordModal() {
          document.getElementById('managerPasswordModal').style.display = 'none';
        }

        function submitManagerPassword() {
          const password = document.getElementById('managerPasswordInput').value;
          const correctPassword = 'Alonissos325*';

          if (password === correctPassword) {
            window.location.href = 'manager-portal.html';
          } else {
            const errorDiv = document.getElementById('managerPasswordError');
        ...
        document.addEventListener('keydown', function(e) {
          if (e.key === 'Escape' && document.getElementById('managerPasswordModal').style.display === 'flex') {
